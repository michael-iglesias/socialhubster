    ...
    $.fn.socialist = function(method) {
                    ...
                    if (settings.feed) {
                        processList.push(helpers.doRequest(settings.feed,"json",function(q){
                            ...
                            for (var i = 0; i < q.data.length; i++) {
                                item = q.data[i];
								var $div = $('<div class="socialist isotope-item"></div>');
        ...
        var helpers = {
            ...
            networkDefs: {
                ...
                facebook:{url:'https://graph.facebook.com/|id|/feed?access_token=173769552824071|dcd4398d5229effdc351088f817514ba&limit=|num|',img:'',dataType:'json',parser:{
                    name: "facebook",
                    ...
                    imgSrcSelector: "(item.picture.split('&url=')[1].replace(/\u00252F/g, '/').replace(/\u00253A/g, ':'))||'/spacer.gif'",
                    imgSrcProcessor: null,
            ...
            timeAgo:function(date_str){
            	var original_date = date_str;
				var date = new Date(Date.parse(date_str));
				var converted = (date.getMonth()+1) + "/" + date.getDate() + "/" + date.getFullYear();
				
				var edited = date_str.split('T')[0];
				
                return edited.split(' ')[0];
            },                
